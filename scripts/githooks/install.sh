...
echo "               push over an open PR head or an existing tag (GH-957)"
echo "runtime   : pre-commit needs bash (Git Bash and Linux both have it);"
echo "            commit-msg and pre-push are POSIX sh"
echo "bypass: git commit --no-verify / git push --no-verify — CI runs on pull"
echo "        requests and pushes to main; a feature branch is gated through"
echo "        its PR's CI Gate. The push guard's own escape is"
echo "        FLEET_ALLOW_FORCE_PUSH=1"
