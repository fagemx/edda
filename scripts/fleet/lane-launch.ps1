...
# GH-694: a name-based UUID (RFC 4122 §4.3 layout: SHA-1 digest, version
# nibble 5, variant 10xx) over the literal `edda-lane-<name>` — the same shape
# scripts/review-pr.sh's review_session_uuid produces, so it is stable across
# relaunches of a lane and distinct between lanes.
function LaneSessionUuid([string]$LaneName) {
  $digest = [System.Security.Cryptography.SHA1]::HashData([System.Text.Encoding]::UTF8.GetBytes("edda-lane-$LaneName"))
...
# GH-694: the claude backend rejects any session id that is not a UUID, so
# the old `lane-<name>` default killed every -Agent claude lane one second
# after launch — before it read its brief, with `agent exited with code 1
# without result` as the only clue.  -DryRun could not catch it: it runs
# Start-Sleep in place of the agent, so the backend never sees the id.
if (-not $SessionId) { $SessionId = LaneSessionUuid $Name }
# A caller-supplied id is checked, never rewritten: silently replacing it
# would break the resume it was passed for.
if ($Agent -eq 'claude' -and $SessionId -notmatch '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$') {
  Fail "-Agent claude requires a UUID session id; '$SessionId' is not one. The backend answers 'Error: Invalid session ID. Must be a valid UUID.' and the lane dies before reading its brief. Omit -SessionId to use the lane's derived UUID."
}
if ($BuildLane) {
...
$argLine = "dispatch --verbose --agent $Agent --prompt-file $(PsQuote $Brief) --session-id $(PsQuote $SessionId) --cwd $(PsQuote $Cwd) --timeout-sec $TimeoutSec"
$argLine += $reviewArgs
