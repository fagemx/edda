...
`crates/edda-cli/src/main.rs:502-507#With --json, exactly one object is printed to stdout:`):
...
`crates/edda-cli/src/main.rs:300-305#Status {`, dispatched at `crates/edda-cli/src/main.rs#Command::Status { json } =>`):
...
and exits `1` (`crates/edda-cli/src/main.rs:1130-1137#if let Err(err) = run(cli) {`).
