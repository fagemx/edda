...
`crates/edda-cli/src/main.rs:533-538#With --json, exactly one object is printed to stdout:`):
...
`crates/edda-cli/src/main.rs:331-336#Status {`, dispatched at `crates/edda-cli/src/main.rs#Command::Status { json } =>`):
...
and exits `1` (`crates/edda-cli/src/main.rs:1166-1173#if let Err(err) = run(cli) {`).
