...
`crates/edda-cli/src/main.rs:493-498#With --json, exactly one object is printed to stdout:`):
...
`crates/edda-cli/src/main.rs:291-296#Status {`, dispatched at `crates/edda-cli/src/main.rs#Command::Status { json } =>`):
...
and exits `1` (`crates/edda-cli/src/main.rs:1121-1128#if let Err(err) = run(cli) {`).
