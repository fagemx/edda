...
async fn scope_whitelist_returns_all_claims() {
    ...
    edda_bridge_claude::peers::write_claim(&pid, "sess-b", "crate-b", &["crates/b/*".to_string()]);
...
async fn scope_whitelist_filters_by_session() {
    ...
    edda_bridge_claude::peers::write_claim(&pid, "sess-y", "crate-y", &["crates/y/*".to_string()]);
...
async fn dashboard_returns_all_sections() {
    ...
    // New summary fields
    assert!(
        json["summary"]["total_cost_usd"].is_f64() || json["summary"]["total_cost_usd"].is_u64()
    );
    assert!(
...
async fn test_sse_stream_since_replay() {
    ...
    // Append two events.
    let ledger = edda_ledger::Ledger::open(tmp.path()).unwrap();
    let e1 = edda_core::event::new_note_event("main", None, "system", "first event", &[]).unwrap();
    ledger.append_event(&e1).unwrap();
...
async fn test_sse_stream_last_event_id_header() {
    ...
    let e2 =
        edda_core::event::new_note_event("main", Some(&e1.hash), "system", "second", &[]).unwrap();
    ledger.append_event(&e2).unwrap();
    drop(ledger);
...
fn cost_anomaly_detection_yellow_and_red() {
    use edda_aggregate::aggregate::{ActivityMetrics, CostMetrics, ProjectMetrics, QualityMetrics};
