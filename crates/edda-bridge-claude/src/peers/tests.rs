...
#[test]
fn branch_identifies_a_session_where_counting_cannot() {
    let pid = "test_branch_identity";
    let _ = edda_store::ensure_dirs(pid);

    // Two live sessions: `infer_session_id` gives up here, which is how a
    // fleet caller ends up with a synthetic id instead of its own (GH-488).
    write_heartbeat_minimal(pid, "sess-a", "worker-a", "/tmp/a");
    update_heartbeat_branch(pid, "sess-a", "feat/a");
    write_heartbeat_minimal(pid, "sess-b", "worker-b", "/tmp/b");
    update_heartbeat_branch(pid, "sess-b", "feat/b");

    assert!(
        infer_session_id(pid).is_none(),
        "counting sessions cannot answer with two live"
    );

    let (sid, label) =
        infer_session_id_on_branch(pid, "feat/b").expect("the branch names exactly one session");
    assert_eq!(sid, "sess-b");
    assert_eq!(label, "worker-b");
}

#[test]
fn branch_matching_never_adopts_a_peers_identity() {
    let pid = "test_branch_no_adopt";
    let _ = edda_store::ensure_dirs(pid);

    // One live peer, and the caller is somewhere else. Counting sessions would
    // hand back the peer; matching the branch refuses.
    write_heartbeat_minimal(pid, "sess-a", "worker-a", "/tmp/a");
    update_heartbeat_branch(pid, "sess-a", "feat/a");

    assert_eq!(
        infer_session_id(pid).map(|(s, _)| s),
        Some("sess-a".to_string()),
        "counting sessions adopts the lone peer -- the hazard being narrowed"
    );
    assert!(
        infer_session_id_on_branch(pid, "feat/mine").is_none(),
        "a caller on another branch is not that session"
    );
}

#[test]
fn two_sessions_on_one_branch_stay_ambiguous() {
    let pid = "test_branch_ambiguous";
    let _ = edda_store::ensure_dirs(pid);

    // Same branch checked out twice is not identity; refusing is the only
    // honest answer.
    write_heartbeat_minimal(pid, "sess-a", "worker-a", "/tmp/a");
    update_heartbeat_branch(pid, "sess-a", "main");
    write_heartbeat_minimal(pid, "sess-b", "worker-b", "/tmp/b");
    update_heartbeat_branch(pid, "sess-b", "main");

    assert!(infer_session_id_on_branch(pid, "main").is_none());
}

#[test]
fn an_unknown_or_empty_branch_matches_nothing() {
    let pid = "test_branch_empty";
    let _ = edda_store::ensure_dirs(pid);
    write_heartbeat_minimal(pid, "sess-a", "worker-a", "/tmp/a");
    update_heartbeat_branch(pid, "sess-a", "feat/a");

    assert!(infer_session_id_on_branch(pid, "").is_none());
    assert!(infer_session_id_on_branch(pid, "no/such/branch").is_none());
}
