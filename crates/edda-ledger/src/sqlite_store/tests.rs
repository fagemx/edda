...
fn event_with_refs() {
    let (dir, store) = tmp_db();
    let mut e = new_note_event("main", None, "system", "with refs", &["decision".into()]).unwrap();
...
fn schema_reopen_recovers_when_v10_columns_exist_but_version_is_stale() {
    let n = COUNTER.fetch_add(1, Ordering::SeqCst);
    let dir = std::env::temp_dir().join(format!("edda_sqlite_partial_{}_{n}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
    let db_path = dir.join("ledger.db");
...
fn schema_migration_v1_to_v2() {
    let n = COUNTER.fetch_add(1, Ordering::SeqCst);
    let dir = std::env::temp_dir().join(format!("edda_sqlite_migrate_{}_{n}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
    let db_path = dir.join("ledger.db");
...
fn a_lock_contention_error_is_retryable_like_a_stale_parent() {
    ...
    // hard failure.
    let stale =
        anyhow::anyhow!("event evt_1 has stale parent_hash: expected Some(\"a\"), got Some(\"b\")");
...
fn append_rejects_broken_parent_hash() {
    ...
    let e2 = new_note_event("main", Some("sha256:bogus_hash"), "system", "broken", &[]).unwrap();
...
fn reopening_repairs_orphan_snapshot_event_idempotently() {
    let n = COUNTER.fetch_add(1, Ordering::SeqCst);
    let dir = std::env::temp_dir().join(format!("edda_snapshot_repair_{}_{n}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
    let db_path = dir.join("ledger.db");
    let store = SqliteStore::open_or_create(&db_path).unwrap();
...
fn temporal_filter_decision_timeline() {
    ...
    store.append_event(&e1).unwrap();
    let mut e2 = make_decision_event_at("main", "db.engine", "postgres", "2026-03-12T00:00:00Z");
...
fn task_lease_v12_migrates_and_round_trips() {
    let n = COUNTER.fetch_add(1, Ordering::SeqCst);
    let dir = std::env::temp_dir().join(format!("edda_sqlite_v13_mig_{}_{n}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
    let db_path = dir.join(".edda").join("ledger.db");
    std::fs::create_dir_all(db_path.parent().unwrap()).unwrap();
...
fn test_schema_v9_to_v10_migration() {
    let n = COUNTER.fetch_add(1, Ordering::SeqCst);
    let dir = std::env::temp_dir().join(format!("edda_sqlite_v10_mig_{}_{n}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
    let db_path = dir.join("ledger.db");
...
fn test_detect_village_patterns_rollback() {
    ...
    let e2 = edda_core::event::new_decision_event("main", Some(&e1.hash), "system", &dp2).unwrap();
    ...
    let e3 = edda_core::event::new_decision_event("main", Some(&e2.hash), "system", &dp3).unwrap();
    store.append_event(&e3).unwrap();
    store
        .conn
        .execute(
