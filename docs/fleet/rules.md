...
- **R6 合併**：LGTM 釘的 SHA 等於 current head、`CI Gate` 綠、SHA 窗為空、P0 與 P1 都是零，就 squash 合併、不刪分支。條件全是機械的，任何控制者都可執行（冪等）；ruleset 18852689 唯一的 required check 是 `CI Gate`，`bypass_actors` 為空——所有代理以同一帳號行動，這關沒有繞過的路。`Independent Review` status（本檔 R18）是判決的機器形式，由 `edda review deliver` 寫入；`scripts/merge-reviewed-pr.sh` 讀它，非 success 就拒絕合併，但它不是 ruleset 的 required check——`review.merge-gate`（2026-09-07 ratified，操作者原話「拆掉 會卡就拆掉」）已把它從 required_status_checks 移除，降為 advisory：merge script 擋，機器閘本身不擋。所以真正不可逾越的只有 `CI Gate`；status 是走 merge script 這條路徑才生效的證據層，不是本條列的機械條件之一。#758 是沒有 status 也合進去的案例，#962 是 #890 用手寫 status 頂替、繞過（當時仍是必要 check 的）本閘的案例——即使現在機器閘已鬆綁，判決本身仍要核對，理由不變。來源：`REVIEW.md` §8「Step 8 — the verdict」、#758、#962、`fleet.merge-authority`、`ci.merge-gate`、`review.merge-gate`。
...
- **R18 獨立審查 status**：`Independent Review` commit status 是判決的機器形式，SHA-pinned。success 只在該 SHA 至少一則判決是 `LGTM (P0=0, P1=0)` 且沒有其他非 LGTM 判決（union 規則）；failure 是有非 LGTM 判決存在；error 是沒有任何判決。只由非作者路徑貼——作者 session 轉貼的判決不翻 status；push 產生新 SHA，新 SHA 沒有 status，舊判決自動作廢。SHADOW 輪次（標題含 ` (SHADOW)`）與 `pr-review-loop` 之類的 self-check 都不是判決，不進 union。來源：`REVIEW.md` §8「Step 8 — the verdict」、#962、本檔 R6。
...
- **R20 L1 歸屬**：fix lane 只跑 L0（touched crates）、push、貼 `Review Response`，不跑 workspace 閘。L1 = exact-head CI（Linux/macOS 全 workspace ＋ Windows 7 crate 子集）＋ verifier 對 Windows 子集外的 touched crate 各做一次 `cargo test -p`，每個凍結 SHA 一次。來源：`.claude/CLAUDE.md` 的驗證階梯、#753。
