...
- **R1 認領**：起手前（R21）在 issue 留 `taking: <machine>/<role> at <ISO8601>` 留言**並**貼 `fleet:claimed` 標籤——留言加標籤合起來才是認領憑證；`lane:*` 標籤（`lane:feature`、`lane:4090`、`lane:docs`）只是路由／分類，永遠不是認領。先寫先贏；讀到別台的 taking 就不開工。來源：`fleet.cross-machine-claim`。
...
- **R21 起手守門**：任何 session 開始一張 issue 之前——`edda dispatch --issue`、fleet-worker 領單、或操作者在對話裡直接指派——先跑 `sh scripts/fleet-claim-issue.sh --check <n> <machine>/<role>`；exit 1（別條 lane 的 `taking:` 已存在，或任何 open／merged PR 指到這張 issue）就不開工，回報原因。檢查與開工在同一個工具步驟；操作者口頭指派的 session 不例外。來源：#A、#716 同機器撞車、R19 的同一原則用在實作側。
