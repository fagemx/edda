...
- 姊妹篇：[策略：五參數，與 edda 的表達方式](2026-09-02-strategy-layer-five-parameters-design.md)——
  本篇定義形狀（單一 lane 的契約），姊妹篇定義策略（形狀之上的組合：群體／多樣性／資訊流／選擇／停止）。
...
**命名註記（審查 Round 1 提出）**：這四個是**概念動詞**，不是現有 CLI 子命令。現有 `edda watch` 是即時 peer／事件 TUI，
`edda intake github` 是外部任務進帳——範圍都比這裡的概念窄。產品化時命名要收斂（歸 #601 README 與 #567 狀態面處理），
在那之前文件裡的 watch／report／promote／intake 一律讀作概念，不要當指令打。
