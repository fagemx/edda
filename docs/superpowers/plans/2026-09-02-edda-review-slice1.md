...
| `crates/edda-cli/src/cmd_review/mod.rs`（create） | `ReviewArgs`、`run()`、`run_inner()`、exit code 對應、人讀輸出 |
| `crates/edda-cli/src/cmd_review/git.rs`（create） | `git()` / `git_ok()` 包裝、base 解析鏈、merge-base、rev-list、worktree add/remove、標記檔 |
    ...
    fn review_verdict_event_type_refs_and_payload() {
        ...
        let payload = ReviewVerdictPayload {
            ...
            reviewer: ReviewReviewer {
                ...
                session_id: "review-bbbbbbbbbbbb-r1".into(), tool_policy: "hard".into(),
            },
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ReviewGateRan { pub cmd: String, pub exit: i32, pub duration_ms: u64, pub stdout_blob: String }
...
mod tests {
    ...
    #[test]
    fn detached_worktree_sees_head_and_marker_then_is_removed() {
        let (_td, root) = testrepo::init();
        testrepo::run(&root, &["checkout", "-q", "-b", "feature"]);
        let head = testrepo::commit_file(&root, "b.txt", "b\n", "feat");
        std::fs::write(root.join("b.txt"), "dirty\n").unwrap(); // author's dirty tree
        let dest = root.join("wt-review");
        git::add_detached_worktree(&root, &dest, &head).unwrap();
        assert_eq!(std::fs::read_to_string(dest.join("b.txt")).unwrap(), "b\n");
        std::fs::write(dest.join(git::SUBJECT_MARKER), &head).unwrap();
        assert_eq!(std::fs::read_to_string(dest.join(git::SUBJECT_MARKER)).unwrap(), head);
        git::remove_worktree(&root, &dest).unwrap();
        assert!(!dest.exists());
    }
}
```
...
pub(crate) fn add_detached_worktree(repo: &Path, dest: &Path, sha: &str) -> Result<()> {
    let dest_s = dest.to_string_lossy().to_string();
    git(repo, &["worktree", "add", "--detach", &dest_s, sha]).map(|_| ())
}

pub(crate) fn remove_worktree(repo: &Path, dest: &Path) -> Result<()> {
    let dest_s = dest.to_string_lossy().to_string();
    git(repo, &["worktree", "remove", "--force", &dest_s]).map(|_| ())
}
```
...
mod tests {
    ...
    fn brief_order_is_trusted_first_diff_last_and_fenced() {
        ...
        assert!(b.text.contains("edda-review-verdict/v1"));
    ...
    fn budget_drops_docs_files_before_code_and_reports_them() {
        ...
        fc.insert("docs/big.md".to_string(), "docs-skills".to_string());
        fc.insert("crates/x/src/lib.rs".to_string(), "code-risk".to_string());
...
Output contract: end your reply with exactly one fenced block:
...
pub(crate) fn assemble(i: &BriefInputs, budget_chars: usize, file_class: &BTreeMap<String, String>) -> Brief {
    let mut chunks = split_diff(i.diff);
    let mut dropped = Vec::new();
    let mut total: usize = chunks.iter().map(|(_, c)| c.len()).sum();
    if total > budget_chars {
        // docs-skills first, largest first; never drop code-risk.
        let mut order: Vec<usize> = (0..chunks.len()).collect();
        order.sort_by_key(|&k| std::cmp::Reverse(chunks[k].1.len()));
        for k in order {
            if total <= budget_chars { break; }
            if file_class.get(&chunks[k].0).map(|c| c == "code-risk").unwrap_or(false) { continue; }
...
**Interfaces:**
- Produces：`pub(crate) struct GateSet { pub cmds: Vec<String>, pub declared_by: Vec<String> }`；`pub(crate) fn gate_set(fm: &FrontMatter, cli_gates: &[String]) -> GateSet`；`pub(crate) fn read_gates(ledger: &Ledger, head_sha: &str, gates: &GateSet) -> (String /*status*/, Vec<ReviewGateRead>, Vec<String> /*uncovered*/)`；`pub(crate) fn normalize_cmd(s: &str) -> String`；`pub(crate) fn extract_probe_cmds(diff: &str, spec: Option<&str>, allow_prefixes: &[String]) -> Vec<String>`；`pub(crate) fn run_probes(cwd: &Path, cmds: &[String]) -> Vec<ReviewProbe>`；`pub(crate) fn spec_trust(explicit_path: bool, trust_flag: bool, pr_author_perm: Option<&str>) -> &'static str`；`pub(crate) fn ran_gates(cwd: &Path, gates: &[String], max_sec: u64, cargo_target_dir_set: bool, ledger_paths: &edda_store::Paths) -> (Vec<ReviewGateRan>, Vec<String> /*notes*/)`；`pub(crate) fn evidence_text(read: &[ReviewGateRead], uncovered: &[String], ran: &[ReviewGateRan], probes: &[ReviewProbe], wiring_scan: Option<&str>) -> String`。
...
mod tests {
    ...
    #[test]
    fn probe_extraction_uses_allowlist_prefixes_only() {
        let diff = "+ run `edda wave --help` then `rm -rf /` and `edda ask x`\n";
        let got = extract_probe_cmds(diff, None, &["edda ".into()]);
        assert_eq!(got, vec!["edda wave".to_string(), "edda ask x".to_string()]);
    }
...
pub(crate) fn gate_set(fm: &FrontMatter, cli_gates: &[String]) -> GateSet {
    let mut cmds: Vec<String> = Vec::new();
    let mut by = Vec::new();
    if !fm.gates.is_empty() { by.push("REVIEW.md".to_string()); }
    if !cli_gates.is_empty() { by.push("--gate".to_string()); }
    for g in fm.gates.iter().chain(cli_gates.iter()) {
...
/// Backticked commands in diff added lines (and spec) whose prefix is allowlisted.
pub(crate) fn extract_probe_cmds(diff: &str, spec: Option<&str>, allow_prefixes: &[String]) -> Vec<String> {
    ...
    for line in sources {
        ...
        while let Some(start) = rest.find('`') {
            ...
            let cmd = normalize_cmd(&after[..end]);
            let cmd = cmd.trim_end_matches(" --help").to_string();
            if allow_prefixes.iter().any(|p| cmd.starts_with(p.as_str())) && !out.contains(&cmd) { out.push(cmd); }
            rest = &after[end + 1..];
        }
    }
    out
...
pub(crate) fn run_probes(cwd: &Path, cmds: &[String]) -> Vec<ReviewProbe> {
    cmds.iter().map(|c| {
        let mut parts = c.split_whitespace();
        let bin = parts.next().unwrap_or("");
        let args: Vec<&str> = parts.chain(std::iter::once("--help")).collect();
        let exit = std::process::Command::new(bin).args(&args).current_dir(cwd)
            .stdout(std::process::Stdio::null()).stderr(std::process::Stdio::null())
            .status().map(|s| s.code().unwrap_or(-1)).unwrap_or(-1);
        ReviewProbe { cmd: format!("{c} --help"), exit }
    }).collect()
}
...
/// Opt-in RAN of declared gates, edda-executed, verbatim, time-capped.
pub(crate) fn ran_gates(cwd: &Path, gates: &[String], max_sec: u64, cargo_target_dir_set: bool, ledger_paths: &edda_store::Paths) -> (Vec<ReviewGateRan>, Vec<String>) {
    let mut ran = Vec::new();
    ...
    for g in gates {
        ...
        if start.elapsed() > Duration::from_secs(max_sec) {
            notes.push(format!("skipped `{g}`: --max-ran-sec {max_sec} exceeded"));
            continue;
        }
        let mut parts = g.split_whitespace();
        let bin = parts.next().unwrap_or("");
        let t0 = Instant::now();
        let out = std::process::Command::new(bin).args(parts).current_dir(cwd).output();
        match out {
            Ok(o) => {
                let tail: Vec<u8> = o.stdout.iter().rev().take(4000).rev().copied().collect();
                let blob = edda_ledger::blob_store::blob_put(ledger_paths, &tail).unwrap_or_default();
                ran.push(ReviewGateRan { cmd: g.clone(), exit: o.status.code().unwrap_or(-1), duration_ms: t0.elapsed().as_millis() as u64, stdout_blob: blob });
            }
            Err(e) => notes.push(format!("failed to spawn `{g}`: {e}")),
        }
    }
    (ran, notes)
}
...
**Interfaces:**
- Produces：`pub(crate) struct Authors { pub sessions: Vec<String>, pub models: Vec<String> /*canonical*/, pub unverifiable: bool }`；`pub(crate) fn authors(ledger: &Ledger, commits: &[String], trailers: &[String]) -> Authors`；`pub(crate) fn independence(a: &Authors, reviewer_session: &str, model_observed: Option<&str>) -> Result<&'static str, String /*refusal*/>` → `Ok("verified" | "same-model" | "unverified")`，`Err` 表示同 session 必須拒絕。
- Consumes：Task 2 的 `canonical_model_id`；Task 5 的 `commits_in_range`；digest 事件形狀（`type=note`，`payload.source="bridge:session-digest"`，`payload.session_id`，`payload.session_stats.model`，`payload.session_stats.commits_made`）。
...
mod tests {
    ...
    #[test]
    fn authors_come_from_digest_commits_made() {
        let (_td, root) = testrepo::init();
        let ledger = edda_ledger::Ledger::open_or_init(&root).unwrap();
        digest(&ledger, "sess-A", "anthropic/claude-opus-5", &["c1", "c2"]);
        digest(&ledger, "sess-B", "gpt-5.6-sol", &["zz"]);
        let a = authors(&ledger, &["c2".into(), "c3".into()], &[]);
        assert_eq!(a.sessions, vec!["sess-A".to_string()]);
        assert_eq!(a.models, vec!["claude-opus-5".to_string()]);
        assert!(!a.unverifiable);
    }

    #[test]
    fn empty_model_or_unknown_trailer_marks_unverifiable() {
        let (_td, root) = testrepo::init();
        let ledger = edda_ledger::Ledger::open_or_init(&root).unwrap();
        digest(&ledger, "sess-A", "", &["c1"]);
        let a = authors(&ledger, &["c1".into()], &["Co-Authored-By: model://weird".into()]);
        assert!(a.unverifiable);
    }
...
pub(crate) fn authors(ledger: &Ledger, commits: &[String], trailers: &[String]) -> Authors {
    let mut sessions = Vec::new();
    let mut models = Vec::new();
    let mut unverifiable = false;
    for ev in ledger.iter_events_by_type("note").unwrap_or_default() {
        if ev.payload["source"] != "bridge:session-digest" { continue; }
        let made: Vec<&str> = ev.payload["session_stats"]["commits_made"].as_array()
            .map(|a| a.iter().filter_map(|v| v.as_str()).collect()).unwrap_or_default();
        if !made.iter().any(|m| commits.iter().any(|c| c.starts_with(m) || m.starts_with(c.as_str()))) { continue; }
...
### Task 10: `run_inner` 端到端組裝與 CLI 級測試（spec §3、§4、§6、§9）

**Files:**
- Modify: `crates/edda-cli/src/cmd_review/mod.rs`

**Interfaces:**
- Consumes：Task 4–9 全部；`crate::agent_kind::{build_launcher, LauncherOptions}`（#574 形狀：`LauncherOptions { verbose, transcript_dir, model, thinking, tools, exclude_tools }`）；`edda_conductor::agent::launcher::{AgentLauncher, PhaseResult}`；`edda_conductor::plan::schema::Phase`；`edda_ledger::Ledger::query_by_paths`；`crate::cmd_dispatch::build_phase`。
- Produces：`pub(crate) fn run_with(args: &ReviewArgs, launcher: &dyn AgentLauncher, gh: &dyn subject::GhClient, cwd: &Path) -> Result<(i32, String /*human*/, serde_json::Value /*json*/)>` — `run_inner` 用真 launcher／`GhCli` 呼叫它；測試用 `MockLauncher` 與 `FakeGh`。

- [ ] **Step 1: 寫失敗測試（端到端，MockLauncher）**
...
mod e2e {
    use super::*;
    use crate::cmd_review::git::testrepo;
    use edda_conductor::agent::launcher::{MockLauncher, PhaseResult};

    fn args(root: &Path, extra: &[&str]) -> ReviewArgs {
        use clap::Parser;
    ...
    #[test]
    fn writes_review_verdict_into_the_author_repo_ledger_and_exits_3_for_unqualified_lgtm() {
        let (_td, root) = testrepo::init();
        ...
        let (code, human, json) = run_with(&args(&root, &[]), &mock, &NoGh, &root).unwrap();
        assert_eq!(code, 3, "{human}");
        assert_eq!(json["verdict"], "lgtm");
        assert_eq!(json["qualified"], false);
        assert!(json["disqualifiers"].as_array().unwrap().iter().any(|d| d == "gates-undeclared"));
        ...
        assert!(!root.join("wt-review").exists());
        let call = &mock.calls()[0];
        assert!(call.session_id.starts_with("review-"));
        assert!(call.prompt.contains("## DIFF"));
        assert!(call.prompt.find("## CORE").unwrap() < call.prompt.find("## DIFF").unwrap());
    }

    #[test]
    fn changes_requested_exits_1_and_unreviewed_on_crash_exits_2() {
        let (_td, root) = testrepo::init();
        edda_ledger::Ledger::open_or_init(&root).unwrap();
        testrepo::run(&root, &["checkout", "-q", "-b", "feature"]);
        let head = testrepo::commit_file(&root, "b.txt", "b\n", "feat");
        ...
        let (c1, _, j1) = run_with(&args(&root, &[]), &mock, &NoGh, &root).unwrap();
        assert_eq!(c1, 1);
        assert_eq!(j1["cost"]["measured"], false);
        let (c2, _, j2) = run_with(&args(&root, &[]), &mock, &NoGh, &root).unwrap();
        assert_eq!(c2, 2);
    ...
    #[test]
    fn subject_mismatch_is_unreviewed() {
        let (_td, root) = testrepo::init();
        edda_ledger::Ledger::open_or_init(&root).unwrap();
        testrepo::run(&root, &["checkout", "-q", "-b", "feature"]);
        let _head = testrepo::commit_file(&root, "b.txt", "b\n", "feat");
        let mock = MockLauncher::new();
        mock.set_results("review", vec![PhaseResult::AgentDone { cost_usd: None, result_text: Some(engine_text("0000", "lgtm")) }]);
        let (code, _, json) = run_with(&args(&root, &[]), &mock, &NoGh, &root).unwrap();
        assert_eq!(code, 2);
        assert_eq!(json["outcome"], "subject-mismatch");
    }

    #[test]
    fn declared_gate_with_receipt_verifies_and_empty_diff_is_exit_2_without_event() {
        let (_td, root) = testrepo::init();
        let ledger = edda_ledger::Ledger::open_or_init(&root).unwrap();
        testrepo::run(&root, &["checkout", "-q", "-b", "feature"]);
        let head = testrepo::commit_file(&root, "b.txt", "b\n", "feat");
        ...
        ledger.append_event(&ev).unwrap();
        let mock = MockLauncher::new();
        mock.set_results("review", vec![PhaseResult::AgentDone { cost_usd: Some(0.01), result_text: Some(engine_text(&head, "lgtm")) }]);
        let (_, _, json) = run_with(&args(&root, &["--gate", "true"]), &mock, &NoGh, &root).unwrap();
        assert_eq!(json["gates"]["status"], "verified");
        // empty diff: reviewing main against main
        let err = run_with(&args(&root, &["--head", "main"]), &mock, &NoGh, &root).unwrap_err();
        assert!(err.to_string().contains("empty diff"));
        assert_eq!(ledger.iter_events_by_type("review_verdict").unwrap().len(), 1);
    }
}
```
...
fn tool_policy(agent: AgentKind) -> (&'static str, Option<Vec<String>>, Option<Vec<String>>) {
    // (tool_policy label, pi --tools allowlist, claude allowedTools)
    match agent {
        AgentKind::Pi => ("hard", Some(vec!["read".into(), "grep".into(), "find".into(), "ls".into()]), None),
        AgentKind::Claude => ("hard", None, Some(vec!["Read".into(), "Grep".into(), "Glob".into()])),
        AgentKind::Codex => ("none", None, None),
    }
}

fn scratch_dir(project_id: &str, head_sha: &str) -> PathBuf {
...
fn run_inner(args: ReviewArgs) -> Result<i32> {
    ...
    let (policy, pi_tools, claude_tools) = tool_policy(args.agent);
    let _ = (policy, &claude_tools);
    let launcher = build_launcher(args.agent, LauncherOptions {
        verbose: false, transcript_dir: None,
        model: args.model.as_deref(), thinking: args.thinking.as_deref(),
        tools: pi_tools.as_deref(), exclude_tools: None,
    })?;
...
pub(crate) fn run_with(args: &ReviewArgs, launcher: &dyn AgentLauncher, gh: &dyn subject::GhClient, cwd: &Path) -> Result<(i32, String, serde_json::Value)> {
    // 1. author repo root and ledger — before any worktree exists
    ...
    if subj.files.iter().any(|f| f == "REVIEW.md") { if !classes.iter().any(|c| c == "docs-skills") { classes.push("docs-skills".into()); } escalations_extra.push("REVIEW.md changed in this diff".to_string()); }

    // 5. ledger pack (existing path query)
    ...
    // 6. evidence
    let gates = evidence::gate_set(&fm, &args.gates);
    let (mut gates_status, read, uncovered) = evidence::read_gates(&ledger, &subj.head_sha, &gates);
    let diff = git::git(&repo, &["diff", &format!("{}..{}", subj.base_sha, subj.head_sha)])?;
    let allow = if fm.ran_allowlist.is_empty() { vec!["edda ".to_string()] } else { fm.ran_allowlist.clone() };
    let probe_cmds = evidence::extract_probe_cmds(&diff, spec_text.as_deref(), &allow);

    // 7. detached worktree + marker
    let wt = scratch_dir(&project_id, &subj.head_sha);
    if wt.exists() { let _ = git::remove_worktree(&repo, &wt); let _ = std::fs::remove_dir_all(&wt); }
    std::fs::create_dir_all(wt.parent().unwrap_or(&wt))?;
    git::add_detached_worktree(&repo, &wt, &subj.head_sha)?;
    std::fs::write(wt.join(git::SUBJECT_MARKER), &subj.head_sha)?;
    let probes = evidence::run_probes(&wt, &probe_cmds);
    let mut notes: Vec<String> = fm_note.into_iter().collect();
    let mut ran = Vec::new();
    if args.run_gates {
        let to_run: Vec<String> = if uncovered.is_empty() { gates.cmds.clone() } else { uncovered.clone() };
        let (r, n) = evidence::ran_gates(&wt, &to_run, args.max_ran_sec, std::env::var_os("CARGO_TARGET_DIR").is_some(), &ledger.paths);
        if !r.is_empty() && r.iter().all(|x| x.exit == 0) && read.iter().all(|x| x.result == "green") && n.is_empty() { gates_status = "verified".into(); }
        if r.iter().any(|x| x.exit != 0) { gates_status = "red".into(); }
        ran = r; notes.extend(n);
    }
    let wiring = if git::git(&repo, &["cat-file", "-e", &format!("{}:scripts/wiring-scan.sh", subj.base_sha)]).is_ok() {
        std::process::Command::new("sh").args(["scripts/wiring-scan.sh", &subj.base_sha, &subj.head_sha]).current_dir(&wt).output().ok()
            .map(|o| String::from_utf8_lossy(&o.stdout).to_string())
    } else { None };
    let evidence_text = evidence::evidence_text(&read, &uncovered, &ran, &probes, wiring.as_deref());

    // 8. brief
    let mut file_class = std::collections::BTreeMap::new();
    for f in &subj.files { for (c, globs) in &classes_map { if brief::route_classes(std::slice::from_ref(f), &[(c.clone(), globs.clone())].into_iter().collect()).len() == 1 { file_class.insert(f.clone(), c.clone()); } } }
    ...
    if b.coverage == "partial" && subj.files.iter().any(|f| file_class.get(f).map(|c| c == "code-risk").unwrap_or(false) && b.dropped_files.contains(f)) {
    ...
    // 9. independence pre-check + run
    let session_id = args.session_id.clone().unwrap_or_else(|| format!("review-{}-r{}", &subj.head_sha[..12], lineage.round));
    ...
    let auth = identity::authors(&ledger, &commits, &trailers);
    if let Err(msg) = identity::independence(&auth, &session_id, None) { let _ = git::remove_worktree(&repo, &wt); anyhow::bail!(msg); }
    let (policy, _pi_tools, claude_tools) = tool_policy(args.agent);
    let mut phase = build_phase(&b.text, args.budget_usd, Some(args.timeout_sec), "bypassPermissions");
    phase.id = "review".into();
    phase.allowed_tools = claude_tools;
    let rt = tokio::runtime::Runtime::new()?;
    let result = rt.block_on(launcher.run_phase(&phase, &b.text, "", &session_id, &wt, CancellationToken::new()));
    let observed = launcher.last_observed_model();
    if !args.keep_worktree { if let Err(e) = git::remove_worktree(&repo, &wt) { notes.push(format!("worktree removal failed: {e}")); } }

    // 10. interpret result
    let (outcome, cost_usd, text) = match result {
        Ok(PhaseResult::AgentDone { cost_usd, result_text }) => ("done", cost_usd, result_text),
        Ok(PhaseResult::AgentCrash { error }) => { notes.push(error); ("crash", None, None) }
        Ok(PhaseResult::Timeout) => ("timeout", None, None),
        Ok(PhaseResult::MaxTurns { cost_usd }) => ("crash", cost_usd, None),
        Ok(PhaseResult::BudgetExceeded { cost_usd }) => ("budget", cost_usd, None),
        Err(e) => { notes.push(e.to_string()); ("crash", None, None) }
    };
    let blobs: Vec<String> = text.as_ref().map(|t| edda_ledger::blob_store::blob_put(&ledger.paths, t.as_bytes()).ok()).flatten().into_iter().collect();
    let (parsed, parse_ok) = match text.as_deref().map(verdict::parse_engine_output) {
        Some(Ok(p)) => (Some(p), true),
        Some(Err(e)) => { notes.push(format!("parse failed: {e}")); (None, false) }
        None => (None, false),
    };
    let mut outcome = outcome.to_string();
    if let Some(p) = &parsed { if p.subject_seen.as_deref() != Some(subj.head_sha.as_str()) { outcome = "subject-mismatch".into(); } }
    ...
    let payload = ReviewVerdictPayload {
        ...
        reviewer: ReviewReviewer { agent: args.agent.as_str().into(), transport: match args.agent { AgentKind::Claude => "claude-code".into(), a => a.as_str().into() }, model_requested, model_observed: model_observed.clone(), observed_via: if observed.is_some() { "in-band".into() } else { "none".into() }, session_id: session_id.clone(), tool_policy: policy.into() },
...
fn render_human(p: &ReviewVerdictPayload, event_id: &str) -> String {
    ...
    s.push_str(&format!("reviewer: {} · requested {} · observed {} ({}) · independence {} (policy {}) · tools {}\n", p.reviewer.agent, p.reviewer.model_requested, p.reviewer.model_observed, p.reviewer.observed_via, p.independence, p.independence_policy, p.reviewer.tool_policy));
...
`MockLauncher` 若沒有 `last_observed_model` 的預設實作，trait 上已由 #574 提供預設 `None`（Task 0 已驗證）。

- [ ] **Step 4: 跑測試確認 PASS**

Run: `cargo test -p edda cmd_review && cargo clippy -p edda --all-targets -- -D warnings`
Expected: 全部 PASS（含 e2e 四個）。
...
```
Expected: 印出一頁人讀輸出；exit 3（`independence-unverified` 等）或 1；帳本有一筆 `review_verdict`；`%TEMP%\edda-review\<project>\` 下沒有殘留 worktree（`git worktree list` 不含它）。

- [ ] **Step 6: Commit**

```bash
git add crates/edda-cli/src/cmd_review/mod.rs
...
`qualified` needs: a spec (`--spec` or a PR closing issue), declared gates (`REVIEW.md` front matter or `--gate`) with receipts at this head, an in-band observed model, verified independence, full coverage, and a hard tool policy (pi or claude). Each missing condition is printed with the action that clears it.
...
Run: `cargo test -p edda deprecation && sh scripts/lint-markdown-content.sh && sh scripts/check-cli-docs.sh 2>/dev/null || true`
Expected: 測試 PASS、markdown lint exit 0（`check-cli-docs.sh` 若尚未由 #650 合入則略過）。
...
對 Task 1、4、7、9、10 各挑一個測試，`git stash` 該 task 的實作（保留測試）跑一次確認 FAIL，`git stash pop`，把五個結果（測試名 ＋ FAIL 訊息一行）記到 PR body 的「Regression proof」段。
...
- **Spec coverage**：§3 CLI（Task 0、10）；§4 主體／worktree／lineage（Task 4、5、10）；§5 brief 與 front matter 與預算（Task 6）；§5.2 契約（Task 9）；§5.3 trust（Task 7）；§6.1 無 shell 工具政策（Task 10 `tool_policy`）；§6.2 in-band model（Task 10 `last_observed_model`）；§6.3 正規化與獨立性（Task 2、8）；§6.4 RAN／probes（Task 7）；§6.5 成本（Task 10）；§7 事件（Task 3、9）；§8 收據（Task 1、7）；§9 失敗表（Task 9、10）；§10 測試（各 task）；§11 deprecation／docs（Task 11）；§12 wiring（Task 12 PR body）。**未覆蓋**：wiring-scan 只在腳本存在時執行（Task 10，已含）；`--pr` 的 `gh` 真實呼叫沒有自動測試（`GhCli` 需網路；Task 10 手動煙霧不含 `--pr`，PR body 記為已知未測）。
- **Placeholder scan**：無 TBD／TODO；所有函式在對應 task 定義。`new_note_event` 的簽名在 Task 5／8 指示執行者以 grep 取得——這是既有 API，不是佔位。
- **Type consistency**：`ReviewGateRead.r#ref`（Task 3）在 Task 7／10 一致；`Lineage.round: u32` 對 `ReviewRefs.round: Option<u32>`（unreviewed → None）；`spec_trust` 回 `&'static str` 直接放 `ReviewSpec.trust`；`tool_policy()` 回三元組在 Task 10 兩處呼叫一致；`MockLauncher::set_results("review", ..)` 對應 `phase.id = "review"`。
