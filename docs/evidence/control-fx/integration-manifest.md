...
| `crates/edda-ledger/src/control_effect_tests.rs` concurrency repair | The two former `#[ignore]`d tests use real OS processes and are enabled for Linux/macOS CI; execution evidence belongs to the exact-SHA PR receipt. Both children open their ledgers and publish readiness before the parent releases the start gate. The effect test presents the same live token to production `Ledger::control_apply_with` and checks one cross-process marker, one applied result, one adopted/refused result, one intent and one receipt. The claim test calls production `Ledger::claim_control_review` with competing signed claims and checks exactly one `Won` and one `RefusedLive`. Child waits are bounded and children are reaped on failure. Only non-blocking workspace admission is retried; production lock, effect and claim semantics are unchanged. Windows does not execute this Unix-gated module |
