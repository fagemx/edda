...
| 事件 | 欄位 | 備註 |
|---|---|---|
| `task.created` | task_id, title, after:[task_ids], brief_ref?, assignee?(agent label), agent_kind?(claude-acp/codex-acp/…), plan_id?, work_unit_ref?, idempotency_key? | DAG 依賴用 `after`;readiness 由投影推導;`work_unit_ref` 對映 wusanto work unit(§8);`?` 欄位可缺——未指派的 task 是合法的待領工作,nudge 與 §8 全鏈只作用於有 assignee 者 |
...
**轉移合法性住在寫入路徑,不在投影**:fold 把事件當事實、永不拒絕——
start 需 ready、done 需 running、收據非空,由 `edda task` 動詞在 workspace
lock 內把關。刻意如此:遲到的 done(原執行者在重排後才完成)是真完成必須
入帳;重複 done 視為更正、最新收據為準;Done 為狀態終態,後續事件不可逆轉。
