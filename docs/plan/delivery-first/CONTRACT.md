...
- 上限 32 KiB；先 lstat，再用平台 no-follow 安全開啟（Unix 同時 nonblocking），
  ...
  read（最多上限 + 1 byte）讀一次到記憶體。Unix identity 是 device+inode；Windows 透過
  pinned safe `same-file` API 取得 volume serial+file index 的 genuine identity，且 opened
  handle 拒絕 write/delete sharing。identity 無法取得即 fail closed。不接受 changed
  identity、directory、任何 symlink/reparse point 或非 regular file；pipe/device 不能阻塞
  或被讀取。
...
每個 Codex product review 都要求 mapping store 可讀，並在回傳前保證最後的 mapping／
tombstone 寫入成功；first round 可接受 map 不存在，但 corrupt／unreadable store 或 lock／
atomic replace/write failure 都是 AgentCrash/refusal，且不寫 review verdict。Codex `--resume`
另外要求既有 mapping；缺 mapping 或 server 拒絕 stale thread 時 fail closed，不可在舊 UUID
下 fresh start。拒絕的 binding 要 durable 移除而不建立 replacement；tombstone persistence
失敗必須連同 rejection 一起回報。Ordinary dispatch 保留 persistence=true、required-
persistence=false、required-thread=false 的 best-effort fallback；conduct 三者皆 false。
Pi／Claude 既有行為不變。
