...
import { MAX_CONTINUATION_INPUT_BYTES, MAX_CONTINUITY_BUNDLE_BYTES, type ContinuationImportRequest, type ContinuationOperation, type ContinuationPublication, type ContinuationPublishRequest, type ContinuationTakeoverRequest, type ContinuationRecoverRequest, type NativeCapsuleInput, type NativeRestore, type PortableBundle } from './continuation-contracts.js';
...
export function nativeContinuityRunner(executable = process.platform === 'win32' ? 'edda.exe' : 'edda'): NativeRunner {
  return (cwd, args, context) => new Promise(resolve => execFile(executable, args, { cwd, shell: false, windowsHide: true, timeout: 15000, maxBuffer: MAX_CONTINUITY_BUNDLE_BYTES * 2, encoding: 'utf8', env: { ...process.env, EDDA_SESSION_ID: 'agent-manager-continuity', ...(context ? { EDDA_ACTOR: context.actor } : {}) } }, (error, stdout) => resolve({ ok: !error, stdout })));
}
...
function provenPrewriteRefusal(kind: 'save' | 'import', status: string, error: string): boolean {
  if (kind === 'save' && status === 'SAVE_FAILED') return ['invalid ContextCapsuleV1 input schema', 'unsupported capsule_version (expected 1)', 'references.event_ids must start with evt_', 'references.event_ids has an invalid identifier shape', 'state.next_action must not be blank'].includes(error) || /^secret content refused in (continuity input|field (input\.|local_source\.))/.test(error);
  if (kind === 'import' && status === 'refused') return ['portable bundle belongs to a different repository', 'invalid portable bundle schema', 'unsupported bundle_version (expected 1)', 'bundle_sha256 does not match bundle content', 'capsule_sha256 does not match capsule bytes', 'capsule_bytes_hex is invalid', 'capsule bytes are not a supported ContextCapsuleV1', 'bundle origin_capsule_id conflicts with capsule bytes', 'bundle portable_repo_id conflicts with capsule bytes', 'capsule bytes must use canonical JSON encoding'].includes(error) || /^secret content refused in (portable bundle|capsule bytes)/.test(error);
  return false;
}
export class ContinuationService {
  ...
  private async capable(workspace: string, operation: 'save' | 'restore' | 'export' | 'import' | 'list') {
    ...
    if (!promise) {
      promise = this.run(workspace, ['continuity', operation, '--help']).then(result => {
        const flags = operation === 'save' ? ['--file', '--json'] : operation === 'restore' ? ['CAPSULE_ID', '--json'] : operation === 'import' || operation === 'list' ? ['--json'] : ['--out'];
  ...
  async latest(id: string): Promise<ContinuationPublication> {
    ...
    const operation = last ? this.readOperation(last) : null;
    if (operation && !['attached', 'failed'].includes(operation.status)) return this.recover(id, { actionId: operation.actionId });
    return this.response(id, operation);
  }
  async recover(id: string, request: ContinuationRecoverRequest): Promise<ContinuationPublication> {
    if (this.closing) throw new ManagerError('STOPPING', '管理服務正在停止，請稍後恢復原操作。', 503);
    const r = object(request), actionId = uuid(r.actionId), selected = r.capsuleId == null ? undefined : capsuleId(r.capsuleId);
    const previous = this.queue.get(id) ?? Promise.resolve();
    const promise = previous.catch(() => {}).then(async () => {
      const state = await this.manager.works.continuationSnapshot(id);
      return this.locks.run(`continuity-task:${state.taskKey}`, async () => {
        const operation = this.readOperation(actionId);
        if (!operation || operation.sourceTaskKey !== state.taskKey || operation.bindingDigest !== hash(JSON.stringify(this.binding(id)))) throw new ManagerError('NOT_FOUND', '原續作操作不屬於此工作或原綁定已改變。', 404);
        await this.reconcile(id, operation, selected); return this.response(id, operation);
      });
    });
    this.queue.set(id, promise); try { return await promise; } finally { if (this.queue.get(id) === promise) this.queue.delete(id); }
  }
  private async reconcile(id: string, operation: StoredOperation, exactId?: string): Promise<void> {
    ...
    if (!selected && (operation.kind === 'save' && operation.marker || operation.kind === 'import' && operation.bundleProof)) {
      ...
      const output = this.parseResult(result.stdout);
    ...
    if (!selected) { operation.notice = '尚未找到可核對的原生 capsule；可提供精確 capsule ID 恢復原操作，未重複保存。'; this.saveOperation(operation); return; }
  ...
  private async performWrite(id: string, actionId: string, revision: string, kind: 'save' | 'import', payload: unknown): Promise<ContinuationPublication> {
    ...
    if (operation) {
      ...
      await this.reconcile(id, operation);
      return this.response(id, operation);
    }
    ...
    if (last && !['attached', 'failed'].includes(last.status)) { await this.reconcile(id, last); if (!['attached', 'failed'].includes(last.status)) throw new ManagerError('CONTINUITY_PENDING', '上一份上下文仍待確認；請先查詢原操作，未重複建立 capsule。', 409); }
    ...
    if (kind === 'save') {
      ...
      marker = `manager:${actionId}:${hash(JSON.stringify(nativePayload))}`;
    ...
    const result = await this.temporary(async file => {
      ...
      return this.run(binding.workspace, kind === 'save' ? ['continuity', 'save', '--file', file, '--json'] : ['continuity', 'import', file, '--json'], marker ? { actor: marker } : undefined);
    });
    let output: Record<string, unknown>;
    try { output = this.parseResult(result.stdout); } catch { await this.reconcile(id, operation); return this.response(id, operation); }
    ...
    if (output.data_authority !== 'data_only' || output.capsule_id == null) {
      ...
      if (provenPrewriteRefusal(kind, status, error)) operation.status = 'failed';
      operation.notice = `${status}：${reason} ${operation.status === 'failed' ? '已確認在寫入前拒絕；可修正輸入後建立新操作，原編號保留。' : '原操作仍保留待核對，不會自動重複寫入。'}`; this.saveOperation(operation);
      return this.response(id, operation);
    }
