...
export class CandidatePanel {
  ...
  private form(candidate: CandidateView): HTMLElement {
    ...
    const cancel = el('button', '取消'); cancel.type = 'button'; cancel.addEventListener('click', () => { this.openForm = ''; this.render(); });
    form.addEventListener('submit', (event) => { event.preventDefault(); void this.register(candidate, { name: nameInput.value, id: idInput.value, projectId: project.value, role: role.value as 'manager' | 'worker' }); });
  ...
  private async register(candidate: CandidateView, fields: { name: string; id: string; projectId: string; role: 'manager' | 'worker' }): Promise<void> {
    if (this.busy) return;
