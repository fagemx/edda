...
// The mailbox a return read resolved to. The label is the opaque root hash. This
// display map is pinned to the server's OWNER_MAILBOX_SOURCE by a test, because
// the browser bundle cannot import `workflow.ts` (node built-ins).
export const OWNER_MAILBOX_LABELS: Record<'binding' | 'env' | 'managed' | 'workspace', string> = { binding: '工作設定指定的 owner mailbox',
